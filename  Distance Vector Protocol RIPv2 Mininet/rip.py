...
#function that reads in 6 json files, each containing the link weights of 
#  the neighboring links to the node's neighbors
#json values are stored as {"link": "weight"}
#must edit metric to take in value from getLinkWeights, and adjust metric
#    according to the cost returned
def getLinkWeights():
    
    u = open('u_link_weights.json')
    uWeights = json.load(u)
    
    v = open('v_link_weights.json')
    vWeights = json.load(v)
    
    x = open('x_link_weights.json')
    xWeights = json.load(x)
    
    w = open('w_link_weights.json')
    wWeights = json.load(w)
    
    y = open('y_link_weights.json')
    yWeights = json.load(y)
    
    z = open('z_link_weights.json')
    zWeights = json.load(z)
    
    linkCosts= [uWeights, vWeights, xWeights, wWeights, yWeights, zWeights]
    
    return linkCosts

#function to implement poison reverse
def poisonReverse():
    # if next sender is previous receiver, 
    # router you are advertising to is the source of your information,
    # therefore must send msg with infinite metric to prevent loop
    return 0
